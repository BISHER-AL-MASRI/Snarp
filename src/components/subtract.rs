...
pub fn subtract_func(vars: &mut HashMap<String, String>, args: Vec<&str>) {
    if args.len() != 3 {
        eprintln!(
            "{}",
            format!("Invalid number of arguments for subtract").red()
        );
        return;
    }
    ...
    if is_debug() {
        println!(
            "subtracting {} and {} to get {} and storing in variable: {}",
